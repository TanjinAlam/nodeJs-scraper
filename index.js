...
async function getTruckDetails(url, html) {
  try {
    await axios.get(url).then(async (response) => {
      ...
      $(offerParam)
        ...
        .get()
        .map((item, index) => {
          let lable = $(item).find(".offer-params__label").text();
      ...
      //   //   .get()
      //   //   .map((item, index) => {
      //   //     console.log("item", index, item.children[0].data);
      //   //   });
      // });
    });
    // });
...
app.get("/results", async (req, res) => {
  await Promise.all(maxPageCount.map(async (index) => scrapPageData(index)));
